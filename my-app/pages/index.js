...
export default function Home() {
  // 1. LET USERS CONNECT THEIR WALLETS

  // 1. keep track of walletConnected
  const [walletConnected, setWalletConnected] = useState(false);

  // 1. create a reference of Web3 Modal to persist as long as the page is open. < to keep metamask connected >
  ...
  const setENSOrAddress = async (address, web3Provider) => {
    ...
    // If the address has an ENS set the ENS or else set the address
    if (_ens) {
      setENS(_ens);
    } else {
      setAddress(address);
  ...
  */
  ...
  // Helper function to rerender the page with setInterval
